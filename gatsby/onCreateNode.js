...
module.exports = exports.onCreateNode = ({node, actions, getNode}) => {
  ...
  switch (node.internal.type) {
    case 'MarkdownRemark':
      ...
      if (!slug) {
        if (relativePath.includes('devlog')) {
          // Blog posts don't have embedded permalinks.
          // Their slugs follow a pattern: /devlog/<year>/<month>/<day>/<slug>.html
          ...
          slug = `/devlog/${year}/${month}/${day}/${filename}.html`

          const date = new Date(year, month - 1, day)

          // devlog posts are sorted by date and display the date in their header.
          createNodeField({
            node,
            name: 'date',
            value: date.toJSON(),
